...
This dataset contains `r nrow(noaa_tidy_df)` weather observations from `r noaa_tidy_df %>% distinct(id) %>% count()` weather stations in NY state from the years `r noaa_tidy_df %>% pull(year) %>% min()` and `r noaa_tidy_df %>% pull(year) %>% max()`. 
